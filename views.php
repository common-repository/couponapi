						...
						<form name="feedSetting" role="form" method="post" action="<?php echo admin_url('admin-post.php'); ?>">
							<div class="row">
								...
								<div class="col-12">
									<div class="form-group">
										<div class="custom-control custom-switch">
											<input type="checkbox" <?php if($config->pingback_setting=='On') { echo 'checked'; } ?> name="pingback_setting" class="custom-control-input" id="pingback_setting">
											<label class="custom-control-label" for="pingback_setting" style = "display:block;"><?= __('Pingback Setting','couponapi')?></label>
										</div>
										<p class=""><span class="dashicons dashicons-bell text-warning"></span> <?= __('Keep this Enabled if your website has low traffic.','couponapi') ?> <span class="text-info" style=" text-decoration: underline; text-decoration-style: dotted; cursor: pointer;" title="<?= htmlspecialchars(__('If your site has low traffic, WP-cron may not trigger automatically, potentially halting the coupon import process. Enabling the "Pingback Setting" periodically triggers WP-cron.', 'couponapi'),ENT_QUOTES,'UTF-8'); ?>"><?= __('Know More?','couponapi') ?></span></p>
									</div>
								</div>
...
?>
	<div>
		...
		<?php } else { ?>
			...
			<div class="mt-4"><h6 class="font-bold text-danger">** <?= __('Daily Usage Limit: 10,000 Logos','couponapi') ?></h6></div>
		<?php } ?>
	</div>
<?php }
